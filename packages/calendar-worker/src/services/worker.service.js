...
async function checkingEvent () {
  ...
  for (let i = 1; i <= 6; i++) {
    ...
    const eventsPromo = await googleApiService.downloadCalendar(
      calendarId[`calendar-all-${groupeText}`]
    )
    const eventTutorat = await googleApiService.downloadCalendar(
      calendarId['calendar-tutorat']
    )
    ...
    const eventsGR1 = await googleApiService.downloadCalendar(
      calendarId[`calendar-gr1-${groupeText}`]
    )
    const eventsGR2 = await googleApiService.downloadCalendar(
      calendarId[`calendar-gr2-${groupeText}`]
    )
    const eventsGR3 = await googleApiService.downloadCalendar(
      calendarId[`calendar-gr3-${groupeText}`]
    )
    const eventsGR4 = await googleApiService.downloadCalendar(
      calendarId[`calendar-gr4-${groupeText}`]
    )

    logger.info('Traitement des différences')
    const gr1 = CheckingBetweenCal(
      eventsPromo,
      eventsGR1,
      '1',
      calendarId[`calendar-gr1-${groupeText}`],
    ...
    logger.info(
      `[Terminé] --> ${nbrAdd} éléments ajoutés et ${nbrSuppr} supprimés\n`
    )
  }
...
async function createCall (event, calendarId) {
  logger.debug(
    `createEvent call : ${event.summary} in ${calendarId} at ${event.start.dateTime}`
  )
...
async function deleteCall (event, calendarId) {
  logger.debug(
    `deleteEvent call : ${event.summary} in ${calendarId} at ${event.start.dateTime}`
  )
